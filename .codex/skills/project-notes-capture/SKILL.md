...
description: Capture and organize project notes in workspace markdown files keyed by `@[project name]`. Use when the user is adding or pasting notes, wants notes grouped by domain then date, wants same-day Outlook email/calendar or Teams context added in italics with a source, needs tracked TODO/milestone/open-question/action updates, or wants `profiles.md` relationship memory maintained from notes, Outlook, and Teams context.
---
...
Also maintain `profiles.md` as the workspace relationship memory for people Andrew interacts with. Use it to preserve company, contact information, role/function, personal details, last known interaction, and interaction style.
...
Do not assign IDs to decisions unless the user explicitly asks later.

Use a workspace-global ID sequence, not a per-project sequence. Scan existing `*_todos.md` files and continue from the highest existing numeric ID across `TODO ####`, `MS ####`, `OQ ####`, and `ACTION ####` entries.

Formatting rules:

- standard TODOs and recurring TODOs should use `TODO 0005`
- milestones should use `MS 0006`
- open questions should use `OQ 0007`
...
Treat all tags inside `# Day Plan` as read-only planning snapshots, not capture instructions. Ignore `TODO`, `RTODO`, `MS`, `OQ`, `DEC`, and `Action` tags found in `# Day Plan`; never create, close, reschedule, or otherwise modify project TODO files from those tags during daily digestion.
...
The `# Day Plan` section often contains copied lists of upcoming TODOs, milestones, recurring TODOs, open questions, suggested TODOs, and delegated actions. Those lists are historical snapshots generated by `day-planning`. During raw-notes ingestion, never interpret any tag or ID in `# Day Plan` as a new instruction or as authority to update due dates/status. Only tags that appear under `# Meeting Notes` are actionable.
...
For raw-notes automation, "do not run on weekends" means do not process notes dated Saturday or Sunday. It does not mean skip a missed Friday capture just because the first available processing time is Saturday, Sunday, or Monday.

For overnight automation or next-morning cleanup:

1. Process only raw-notes files whose file date is Monday through Friday.
2. Run at the first available time even when the current day is Saturday or Sunday.
3. Starting from the most recent prior calendar day, scan backward for the latest existing weekday raw-notes file that has not already been captured.
4. Skip Saturday and Sunday raw-notes files unless the user explicitly asks to process them.
5. If no unprocessed weekday raw-notes file exists in the recent lookback window, stop without guessing a different date.
6. Before writing anything, check whether the candidate raw-notes file appears to have already been captured into the target project notes.
7. First read each target project's `## Raw Notes Capture Status` section when present.
8. Treat a meeting section as already captured when either:
...
9. If every meeting section in the raw-notes file appears already captured, continue scanning backward for another unprocessed weekday file before stopping.
10. If only some meeting sections appear already captured, skip those sections and ingest only the remaining ones.
11. Use the same rule set for any uncaptured sections: ignore `# Day Plan`, process only `# Meeting Notes`, and route each meeting separately by `@[project name]`.
...
During end-of-day or raw-notes digestion, explicitly look for Outlook emails that contain meeting minutes, meeting notes, recaps, decisions, action items, or follow-up summaries related to each meeting section being ingested. Do this as a distinct enrichment step, not merely as a generic email search.

Give special attention to meeting-minutes emails from Maggie Stahl and Soumajit Sen because they are PMs who often send recaps. When either person is an attendee, project PM, sender in recent related threads, or otherwise likely to own the recap:

- search Outlook email for the meeting title plus their name
- search Outlook email for the project plus their name
- search Outlook email for distinctive meeting phrases plus terms such as `minutes`, `meeting notes`, `recap`, `follow up`, `follow-up`, `actions`, `action items`, `decisions`, or `notes`

Use a same-day window by default. If processing yesterday's notes, Friday notes, or an older raw-notes file, also check the next business day when practical because PM meeting minutes are often sent after the meeting or the following morning.

When a matching email contains meeting minutes or a meeting recap:

- summarize the email faithfully under the relevant meeting cluster as added context
- preserve concrete decisions, action owners, dates, and open issues
- do not replace, compress, or rewrite Andrew's original notes
- italicize the added context and cite the email subject, sender, and sent or received time inline
- if the email has detailed minutes that materially expand the raw notes, include the important details rather than reducing them to a vague one-line summary
...
## Maintain the people profiles file

When digesting raw notes or capturing new notes, update `profiles.md` whenever the notes, Outlook, Teams, or meeting context reveal meaningful people information.
...
- description of Andrew's interaction style with the person

Organize profiles by the company the person works for. If the company is unclear, use `Unknown / To classify` and add the uncertainty in the entry.

Mark role/function assumptions in italics. Only make an assumption when the evidence is strong enough to be useful. Otherwise write `Unknown` and let future captures improve it.

For external contacts, do not leave contact information as `Unknown` if Outlook likely contains a signature or message header. Before finalizing a new or updated external profile, search Outlook by the person's name, company name, and email domain when practical. Use signatures and headers to capture email address, phone/direct numbers, office numbers, title, and location. If a person only appears as a copied stakeholder, say that rather than treating them as a primary owner.

Update `Last known interaction` when the current note, same-day Outlook context, or Teams context shows a newer meaningful interaction than the existing profile. Preserve older personal details and relationship notes unless they are clearly corrected by newer information.

Do not create profile entries for mass distribution lists, newsletters, automated senders, room/resource mailboxes, or people who only appear as incidental attendees with no meaningful working interaction. Prefer direct interactions, repeated project interactions, 1:1s, interview notes, external partners, and people Andrew may reasonably ask about later.

When a person is a temporary employee or contractor excluded from Andrew's formal people map, the profile may still be captured if there is useful working context, but label the entry clearly as temporary/contractor working context and do not treat them as a formal direct report.

Treat personal details as private context for Andrew's memory and meeting prep. Do not include personal details in external emails, Teams messages, or formal artifacts unless Andrew explicitly asks.

If a note says an open question has been answered by a person or an action has moved, update both the relevant TODO file and the person's profile when the interaction gives useful context.
...
Detect open-question tags in the form `[OQ]`.
...
If the user's prompt is only a standalone milestone or open-question entry, such as just `MS_YYYY-MM-DD ...` or just `[OQ] ...`, do not create or append a notes cluster for that prompt. In that case, update only `[project name]_todos.md`.
...
- the stable ID
- the owner or intended respondent when clear from the question text, attendee context, or surrounding notes
- `Unassigned` or `Owner unspecified` when no owner is clear
- the open-question text
- capture date
- status as open unless the user later closes it
- a pointer to the notes date/domain when practical

Format open-question entries as `- [ ] OQ 0007 - Owner or Unassigned - Question text. Captured YYYY-MM-DD. Source: ...`.
...
When a user supplies `[OQ]`, treat the rest of the line or bullet as the open-question text. Do not require a person name in the tag.

Before adding or updating an open-question entry:

1. Read the existing `## Open Questions` section if it exists.
2. Assign the next workspace-global stable ID in the form `OQ 0007`.
3. If the open-question text names a person or shorthand, normalize that person through the alias map when possible.
4. Compare any inferred owner with names already tracked in `## Open Questions`.
5. If the inferred owner is not an exact or alias-normalized match but looks like an abbreviation, partial name, or otherwise plausible reference to an existing person, ask one short clarifying question before changing the file.
6. Only proceed without asking when the owner is either an exact existing match, a clear alias-map match, clearly a new distinct person, or not specified at all.
...
When processing raw daily notes, limit all tag detection and close/answered-item checks to the parsed `# Meeting Notes` content. Ignore `# Day Plan` content entirely for tag detection, due-date updates, close requests, and open-question resolution, even if it contains valid-looking tags or stable IDs.
...
5. Preserve the original question text and `OQ ####` ID rather than rewriting it.

When the user says a specific TODO is closed:

1. If the user gives an ID such as `close TODO 0005`, `close OQ 0007`, `close ACTION 0012`, or `close 0005`, search all project TODO files for that ID first.
...
If the user supplies an open-question ID such as `close OQ 0007`, prefer that exact ID over text matching.
...
11. For each uncaptured meeting section, do a deliberate same-day enrichment pass:
   ...
   - explicitly search Outlook email for meeting-minutes or recap emails tied to that meeting, prioritizing Maggie Stahl and Soumajit Sen when relevant
...
When the user sends only a standalone `MS_YYYY-MM-DD ...` or `[OQ] ...` line:
...
- any `TODO_YYYY-MM-DD`, `MS_YYYY-MM-DD`, or `[OQ]` tags
